...
|기능|설명|
|----|---|
|뇌파 데이터 수집 및 학습|- 블루투스를 이용하여 안드로이드 폰과 뇌파기기 연결</br> - 뇌파기기의 연결 강도를 나타내어 뇌파와 어플이 연결이 잘 되었는지 보여줌</br> - 운전 시작 버튼을 누르면 뇌파가 수집되고 운전 종료 버튼을 누를 시 뇌파 수집이 멈춤</br> - 운전자로부터 받아오는 뇌파 데이터의 상태를 어플에서 실시간으로 확인 가능</br> - 뇌파 데이터를 그래프로 시각화시켜 뇌파에 생소한 운전자도 쉽게 알아볼 수 있도록 제작</br> - 뇌파의 Alpha, Theta, Delta, Beta, Gamma, Attention, Meditation, Blink값을 색별로 분류하여 나타냄|
|운전 환경 표시|- 홈화면 상단에 현재 위치에 따른 주소, 날씨, 온도, 습도, 풍속을 알려줌</br> - 상단에 날씨에 따른 기상 이미지를 넣어 직관적으로 날씨를 알려줌</br> - 상단에 운전자의 등록된 이름과 함께 ‘환영합니다’ 라는 메시지로 환영 인사|  
|문자 전송 기능|- 회원가입 시 운전자의 보호자 전화번호를 입력함</br> - 운전자의 졸음이 2회 이상 감지될 경우 인근 휴게소로 안내해주며 보호자에게 운전자가 이동하는 휴게소와 졸음 상태를 문자로 전송함</br> - 운전자의 졸음이 중지되면 본 어플에서 제공하는 졸음 퇴치 기능 이외에도 보호자가 운전자에게 전화 혹은 다른 기능들을 통하여 운전자를 졸음에서 벗어나도록 도와줄 수 있음</br> - 뿐만 아니라 운전자가 원치 않는 사람의 번호를 입력할 시 연락이 가지 않도록 졸음에 더 주의하여 운전을 할 수 있는 효과가 있음|  
|알람 재생|- 졸음운전 시 운전자의 졸음을 쫓고 경각심을 알리기 위해 알람 재생</br> - “알람을 종료하려면 ‘알람 꺼줘’라고 말해주세요” 라는 문구를 제공해 운전자가 의사결정의 주체가 되도록 함</br> - 운전자의 졸음 감지 시 해당 시점의 환경 및 데이터를 저장|  
|인근 휴게소 안내|- 구글맵 API를 통해 졸음 상태에 따라 실시간 인근 휴게소 알림</br> - 휴게소 마커 클릭 시 휴게소 정보 안내</br> - 휴게소 정보 누를 시, 해당 휴게소로 카카오네비 API를 통해 네비게이션 시작</br> - “oo휴게소로 길 안내를 해드릴까요? 원하시면 ‘해줘’, 아니면 ‘됐어’라고 말해주세요”라는 안내멘트를 제공하여 운전자가 의사결정의 주체가 되도록 함|
|졸음 리포트|- 졸음 시기의 날짜, 해당 시간 확인 가능함</br> - 졸음 시기의 위치, 날씨, 습도, 온도, 풍속, 공기 질을 확인 가능하도록 제작</br> - 졸음 시기의 위치를 지도에 나타내어 한눈에 알아보기 쉽도록 제작|
|회원 정보 수정|- 설정 페이지에서 회원정보를 누르면 회원정보 확인 가능</br> - 회원 정보 새로 작성 후 ‘수정하기’ 버튼을 누르면 DB에 정보로 업데이트됨|  
...
|상태|train|validation|test|total|
|----|---|:---:|---|---|
|정상 상태|276,480|92,160|92,160|460,800|
|졸음 상태|184,320|61,440|61,440|307,200|  
